...
- If user invokes `html-report` with text after it, treat that trailing text as the request.
- Reserve `html-report recap` and `html-report recap+` as the only special modes.
- If user invokes bare `html-report` with no trailing request, reply in chat with concise explanation of valid modes.

## MODE

- `html-report <user request>`
  - Format your next reply as a full HTML doc based on user request.
  - Treat any text after `html-report` as the request, including plain phrases like `break it down`.
...
- Save exports under `.html-report/` in workspace root.
- Filename format: `html-report-YYYYMMDD-HHMMSS.html`.
